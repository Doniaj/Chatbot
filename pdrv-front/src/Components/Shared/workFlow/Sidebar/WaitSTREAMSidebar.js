...
const WaitSTREAMSidebar = ({ selectedNode, setNodes, goBackToDefault }) => {
    ...
    const [timeoutStream, setTimeoutStream] = useState(30);
    ...
    useEffect(() => {
        if (selectedNode) {
            ...
            setLabel(data.label || "");
            setTimeoutStream(data.timeoutStream || 30);
    ...
    return (
        <div className={`sidebar-container ${themeClass}`}>
            ...
            <h3  className="sidebar-tools" >Wait STREAM Node</h3>
            ...
            <div className="form-group">
                <label>Timeout Stream (secondes) :</label>
                <input
                    type="number"
                    min="1"
                    max="60"
                    value={timeoutStream}
                    onChange={(e) => setTimeoutStream(Number(e.target.value))}
...
export default WaitSTREAMSidebar;